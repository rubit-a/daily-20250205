...
- [x] Post 상세 조회 시 Comments N+1 발생 확인 (11개 쿼리: 1 + 10 Comments)
...
### 인덱스 효과 검증
- [x] 테스트 데이터 삽입 (사용자 100명, 게시글 1000개, 댓글 5000개)
- [x] H2 Console에서 `EXPLAIN` 실행
- [x] 인덱스 사용 여부 확인
- [x] 복합 인덱스 컬럼 순서 영향 테스트
...
- [x] `Pageable` 파라미터 적용
- [x] `Page<T>` 반환 (총 개수 포함)
- [x] `Slice<T>` 반환 (총 개수 없이 - 더 효율적)

### 정렬
- [x] `Sort.by("createdAt").descending()` 적용
- [x] 복합 정렬 테스트

### 커버링 인덱스
- [x] 페이지네이션 + 정렬이 인덱스를 타는지 확인
...
- [x] 쿼리 실행 시간 로깅
- [x] 대량 데이터에서 인덱스 유무 비교
- [x] Fetch Join vs @BatchSize 성능 비교
